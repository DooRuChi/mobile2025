...
class MainActivity : AppCompatActivity() {
    ...
    companion object{
        private const val TAB = "KotlinWeek06App"

    }
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_main)
        Log.d(TAB, "onCreate: AppWeek05 started")
        // functions
    ...
    private fun setupListener(){
        ...
        Log.d(TAB, "Event listeners setup complted")
    }
    private fun addStudent(){
        ...
        if(studentName.isEmpty()){
            ...
            Log.d(TAB, "Attempted to add empty student name")
            return
        }
        ...
        updateStudentCount()
        Toast.makeText(this, "Added: $studentName (Total: ${studentList.size})", Toast.LENGTH_SHORT).show()
    }
    private fun clearAllStudents(){
        ...
        Log.d(TAB, "Cleared all students (Total cleared: $count)")
    }
    private fun removeStudent(position: Int){
        if(position >= 0 && position < studentList.size){
            ...
            Log.d(TAB, "Removed student: $removedStudent (Remaining: ${studentList.size})")
        }
    }
    private fun updateStudentCount(){
        textViewCount.text = "Total Students : ${studentList.size}"
    }
    private fun addInitialDate(){
        val initialStudents = listOf("Kim", "Lee", "Park")
        studentList.addAll(initialStudents)
        adapter.notifyDataSetChanged()
        updateStudentCount()
        Log.d(TAB, "Added initial data: $studentList")
    }

    override fun onResume() {
        super.onResume()
        Log.d(TAB, "onResume: Current student count: ${studentList.size}")
    }

    override fun onPause() {
        super.onPause()
        Log.d(TAB, "onPause: Saving state with: ${studentList.size} students")
    }
}
